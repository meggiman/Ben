...
/**
 * Diese Klasse stellt Methoden bereit, um verschieden {@link ReversiPlayer} automatisiert gegeneinander antreten und lernen zu lassen.
 * Die Klasse benutzt nur einen Thread. Die Einhaltung der Timelimits wird erst nach Rückgabe eines Zuges durch die {@code ReversiPlayer} geprüft.
 * Die Testprogramme können also durch zu lange Berrechnungen stark ausgebremst oder zum Absturz gebracht werden.
 *
 */
public class TestArena {
	...
	/**
	 * 
	 * @param player1 Der {@code ReversiPlayer}, welcher abhängig vom gewähltem Testmodell nur die Farbe grün spielt.
	 * @param player2 Der {@code ReversiPlayer}, welcher abhängig vom gewähltem Testmodell nur die Farbe rot spielt.
	 * @param timelimit Das Zeitlimit, welches die beiden Spieler haben.
	 */
	 ...
	 * @param player1 Der {@code ReversiPlayer}, welcher abhängig vom gewähltem Testmodell nur die Farbe grün spielt.
	 * @param player2 Der {@code ReversiPlayer}, welcher abhängig vom gewähltem Testmodell nur die Farbe rot spielt.
	 * @param timelimitplayer1 Das Zeitlimit, welches der Spieler Rot hat.
	 * @param timelimitplayer2 Das Zeitlimit, welches der Spieler Grün hat.
	 */
	...
	public TestResult normalGame(int player1colour,int nrofgames){
		...
		this.player2colour = (player1colour == Bitboard.RED) ? Bitboard.GREEN: Bitboard.RED;
...
//		}
	...
	public class TestResult{
		...
		public void finishtest(){
			averageNrofsearchedNodes1 = totalNrofsearchedNodes1/NrofmadeMoves1;
			averageNrofsearchedNodes2 = totalNrofsearchedNodes2/NrofmadeMoves2;
			averageNrofevaluatedNodes1 = totalNrofevaluatedNodes1/NrofmadeMoves1;
			averageNrofevaluatedNodes2 = totalNrofevaluatedNodes2/NrofmadeMoves2;
			...
			for (GameResult game : gameResults) {
				...
				for (int i = 0; i < game.moveorderingcount1.size(); i++) {
					moveOrderingcount1.set(i, moveOrderingcount1.get(i)+game.moveorderingcount1.get(i));
				}
			}
			for (GameResult game : gameResults) {
				...
				for (int i = 0; i < game.moveorderingcount2.size(); i++) {
					moveOrderingcount2.set(i, moveOrderingcount2.get(i)+game.moveorderingcount2.get(i));
				}
			}
			moveorderingpercentage1 = new float[moveOrderingcount1.size()];
			for (int i = 0; i < moveorderingpercentage1.length; i++) {
				moveorderingpercentage1[i] = moveOrderingcount1.get(i)/(float)NrofmadeMoves1;
			}
			moveorderingpercentage2 = new float[moveOrderingcount2.size()];
			for (int i = 0; i < moveorderingpercentage2.length; i++) {
				moveorderingpercentage2[i] = moveOrderingcount2.get(i)/(float)NrofmadeMoves2;
			}
		}
		...
		private class xmlwriter{
			...
			private void writemoves(XMLStreamWriter writer, ArrayList<MoveResult> moves) throws XMLStreamException{
				...
				for (int i=0; i< moves.size();i++) {
					...
					writer.writeAttribute("playernr", (moveResult.player==PLAYER1)?"Player1":"Player2");
		...
		public class GameResult{
			public static final int REGULARRESULT = 0, TIMEEXCEEDED = 1, ILLEGALMOVE = 2, EXCEPTION = 3;
