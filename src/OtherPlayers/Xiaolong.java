...
public class Xiaolong implements ReversiPlayer{
  ...
  private long timeLimit, nodes, hashhits, hashmisses, startTime, hashkey;
  private boolean aborted = false;
  private static int HASHSIZE = 0x800000; // 64 MB
  private static int HASHSIZEAND; // 23 bits
  private final static int HASHITER = 0x1;
  private final static int TYPEAND = 0x3; // 2 bits
  private final static int DEPTHAND = 0x7F; // 7 bits
//  private final static int MOVEAND = 0x7F; // 7 bits
  private final static int VALUEAND = 0x7FFF; // 15 bits
  ...
  private final static int INFINITY = 15000; // less than 2^14
  // maximum valuation
  private final static int MATE = 10000; // less than 2^14 - 512
...
//  private final static long PM2 = 0x0000182424180000L;
  ...
  /*
   * originally from Software Optimization Guide for AMD Athlon™ 64 and Opteron™ Processors
  ...
  private int evaluate(){
    ...
    return (((p1 | p2) & PMO) == 0 ? (realDepth - parallelCount(p2) < 2 ? -30 : 0) : 36 * parallelCount(getStableDisks(p1, p2)) - 38 * parallelCount(getStableDisks(p2, p1)))
            + 8 * (parallelCount(getBorders(p2, p1))) - 10 * (parallelCount(getBorders(p1, p2)))
            + 4 * (parallelCount(getMoves(p1, p2))) - 5 * (parallelCount(getMoves(p2, p1)))
  ...
  public void initialize(int i, long l){
    ...
    timeLimit = l - 25; // -25 ms less as a sufficient time buffer for abortion
    hashkey = 0;
    color = i;
    BB[0] = 0;
    BB[1] = 0;
    LS[0] = -1;
    LS[1] = -1;
    System.out.println(this.getClass().getSimpleName() + " is " + (color == GameBoard.GREEN ? "green" : "red"));
    initHashTable();
    initMoveList();
  }
  ...
  private Coordinates search(){
    ...
    if(availableMoves != 0){
      ...
      for(; !aborted; depth += 2){
        ...
        if(aborted || ((System.currentTimeMillis() - startTime) > timeLimit)){
          aborted = true;
          break;
        }else{
          if((maxValue < MATE || bestValue >= maxValue) && bestIndex != -1){
            myMove = new Coordinates((bestIndex / 8) + 1, (bestIndex & 0x7) + 1);
            maxValue = bestValue;
            maxDepth = depth;
          }
  ...
  private void shuffleMoves(){
    Random r = new Random();
    int f = 0, p = 0, t = 0, i, pos[] = {0, 4, 8, 16, 24, 32, 40, 48, 56, 60}, tmp, in;
    for(i = 0; i < 60; i++){
      if(pos[p] == i){
        f = pos[p];
        p++;
  ...
  public Coordinates nextMove(GameBoard gb){
    ...
    System.out.printf("Valuation: %d, depths complete: %d, time used: %.3fs, total nodes: %.3fkn, nodes/s: %.3fkn/s, hashhits: %.1f%%\n", maxValue, (maxDepth + cD == 64 ? 64 - cD : maxDepth + 1), (elapsedTime * 0.001), (nodes * 0.001), (nodes * 1.0) / (elapsedTime * 1.0), (hashhits * 100.0) / (hashhits + hashmisses));
    return myMove;
  }
  ...
  private int pvsearch(int depth, int alpha, int beta, boolean passed){
    ...
    nodes++;
    if(aborted || ((nodes & CHECKTIME) == 0 && (System.currentTimeMillis() - startTime) > timeLimit)){
      aborted = true;
      return 0;
    }
  ...
  private void initHashTable(){
    ...
    System.out.printf("hashtable memory in use: %.1fMB\n", (HASHSIZE * 8.0) / (1024 * 1024));
    HASHPLAYERXOR[0] = (r.nextLong() << 21) ^ (r.nextLong() >>> 35) ^ (r.nextLong() << 4);
    HASHPLAYERXOR[1] = (r.nextLong() << 21) ^ (r.nextLong() >>> 35) ^ (r.nextLong() << 4);
    for(i = 0; i < 64; i++){
      HASHFLIPXOR[0][i] = (r.nextLong() << 21) ^ (r.nextLong() >>> 35) ^ (r.nextLong() << 4);
      HASHFLIPXOR[1][i] = (r.nextLong() << 21) ^ (r.nextLong() >>> 35) ^ (r.nextLong() << 4);
      HASHMOVEXOR[0][i] = (r.nextLong() << 21) ^ (r.nextLong() >>> 35) ^ (r.nextLong() << 4);
      HASHMOVEXOR[1][i] = (r.nextLong() << 21) ^ (r.nextLong() >>> 35) ^ (r.nextLong() << 4);
    }
  }

  private void store(int depth, int alpha, int beta, int bestValue, int bestIndex){ // could be extended with move usage, no use since mpc table got lost
    ...
    for(i = 0; i < HASHITER; i++){
      ...
      if(hash == 0 || (int)(hash >>> 32) == key){
        ht[index + i] = ((long)key << 32) | (type << 29) | ((bestValue + INFINITY) << 14) | (bestIndex << 7) | depth;
        return;
      }
    ...
    ht[index + lowestIndex] = ((long)key << 32) | (type << 29) | ((bestValue + INFINITY) << 14) | (bestIndex << 7) | depth;
  }
